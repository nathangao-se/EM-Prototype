...
window.WizardSteps = (function () {
  ...
  function buildEntityTreeHTML(nodes, depth, ctx) {
    ...
    nodes.forEach(function (node) {
      ...
      if (hasChildren) {
        html += '<button class="inv-tree-toggle" data-ws-toggle="' + node.id + '">' +
          '<i class="fa-solid fa-chevron-' + (isExpanded ? 'down' : 'right') + '"></i></button>';
      } else {
  ...
  function bindEntitiesStep(ctx, onChange) {
    ...
    b.querySelectorAll('.inv-tree-toggle[data-ws-toggle]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        if (icon) icon.className = ctx.entityExpanded[id] ? 'fa-solid fa-chevron-down' : 'fa-solid fa-chevron-right';
      });
    });
  ...
  function buildAssignEntityTreeHTML(nodes, depth, ctx) {
    ...
    nodes.forEach(function (node) {
      ...
      if (hasChildren) {
        html += '<button class="inv-tree-toggle" data-ws-at-toggle="' + node.id + '" style="width:20px;font-size:10px">' +
          '<i class="fa-solid fa-chevron-' + (isExpanded ? 'down' : 'right') + '"></i></button>';
      } else {
